...
import * as ConnectionIdReducer from './../reducers/connectionid.reducer';
...
describe('AppstateService', () => {
  ...
  describe(MessageTypes.Testresults, () => {
    it('should set/sync all stored test results to the received results', async () => {
      ...
      const expectedNumberOfRecords = constants.TEST_RESULTS_SITE_1_AND_2.payload.reduce((a,c) => a + c.length, 0);

      await expectWaitUntil (
        null,
        () => {
          const stdfRecordsStoredOnService = service.stdfRecords.reduce((a,c) => a + c.length, 0);
