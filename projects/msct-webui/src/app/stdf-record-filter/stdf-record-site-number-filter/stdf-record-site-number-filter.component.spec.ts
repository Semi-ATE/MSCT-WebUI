...
describe('StdfRecordSiteNumberFilterComponent', () => {
  ...
  async function waitProperInitialization(): Promise<void> {
    const expectedLabelText = 'Show only the following sites';
  ...
  describe('Site Filter', () => {
    ...
    describe('Checkbox', () => {
      it('should be rendered in DOM', async () => {
        const containedString = 'Show only the following sites';
        const found = (): boolean => {
      ...
      it('should call function updateFilterAndPublish when clicked', async () => {
        ...
        const spy = spyOn<any>(component, 'updateFilterAndPublish');
        debugElement
    ...
    describe('siteNumberFilterCheckboxChanged', () => {
      it('should call updateFilterAndPublish', () => {
        const updateFilterSpy = spyOn((component as any), 'updateFilterAndPublish');
    ...
    it('should only show stdf records of site 0 if site filter is active and configured to see only site 0', async () => {
      ...
      const allrecords = filterService.filteredRecords.reduce( (a,c) => a.concat(...c), []);
