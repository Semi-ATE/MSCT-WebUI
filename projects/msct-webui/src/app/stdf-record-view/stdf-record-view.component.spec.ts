...
describe('StdfRecordViewComponent', () => {
  ...
  const conditionRecord = (head: number, site: number): boolean => {
    const record = debugElement.query(By.css('app-stdf-record'));
  ...
  describe('currentRecord', () => {
    it('should return Unknown Ptr in case of an invalid index', () => {
      ...
      filterService.filteredRecords = [];
      const expectedResult = { type: StdfRecordType.Unknown, values: [] };
      const result = component.currentRecord();
  ...
  describe('autoscroll', () => {
    ...
    it('should change current record automatically in case that a new record arrives', async () => {
      ...
      const record1 = {type: 'PIR', HEAD_NUM: 0, SITE_NUM: 0};
      const record2 = {type: 'PIR', HEAD_NUM: 1, SITE_NUM: 1};
  ...
  describe('navigation buttons', () => {
    it('should render navigation buttons "prev" and "next" if option autoscroll is turned off', async () => {
      ...
      const autoscrollChanged = spyOn(component, 'autoscrollChanged').and.callThrough();

      // turn autoscroll off
      debugElement
        .queryAll(By.css('lib-checkbox'))
      ...
      const enabledPrevButton = (): boolean => {
      ...
      // click prev button
      debugElement.query(By.css('#prev button')).nativeElement.click();
    ...
    it('should have hidden class depending on the autoscroll state, i.e. hidden if autoscroll is off', () => {
      ...
      const stdfRecord = { type: StdfRecordType.Pir, values: [{key: 'HEAD_NUM', value: 0}, {key: 'SITE_NUM', value: 0}]};
  ...
  it('should render message "Filter settings are too restrictive! No records match the applied filter settings." if filter settings are too strong', async () => {
    ...
    await expectWaitUntil(
      ...
      () =>
        debugElement
          .queryAll(By.css('.stdfRecordTypes lib-checkbox'))
          .filter((e) =>
  ...
  describe('reloadRecords', () => {
    it('should send command "getresults" to backend in order to request all stored records', () => {
      ...
      const sendArguments = [];
      const sendSpy = spyOnStoreArguments(communicationService, 'send', sendArguments);
