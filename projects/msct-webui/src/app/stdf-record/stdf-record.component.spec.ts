...
import { StdfRecordType, StdfRecordProperty, STDF_RECORD_ATTRIBUTES, Stdf } from './../stdf/stdf-stuff';
...
describe('StdfRecordComponent', () => {
  ...
  it('should show all entries', () => {
    const expectedEntries: StdfRecordProperty[] = [
      {key: 'label1', value: 1},
    ...
    const expectedStrings = expectedEntries.map(e => e.key + ':\n' + e.value);
    const entries = debugElement.queryAll(By.css('.recordEntries li'));
    const currentStrings = entries?.map(e => e.nativeElement.innerText);
  ...
  describe('computeMultichoiceConfigurationForTestFlagValue', () => {
    it('should compute label starting with "Test Flag"', () => {
      const configuration = component.computeMultichoiceConfigurationForTestFlagValue(1);
    ...
    it('should always compute 8 items', () => {
      ...
      for (let i = 0; i < numbers.length; ++i) {
        const configuration = component.computeMultichoiceConfigurationForTestFlagValue(numbers[i]);
