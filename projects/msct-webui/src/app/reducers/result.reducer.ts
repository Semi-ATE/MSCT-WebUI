...
const reducer = createReducer(
  ...
  on(ResultActions.addResult, (state, {prr}) => {
    const key: SiteHead = computeSiteHeadFromRecord(prr);
    state.set(key, prr);
...
export function resultReducer(state: Map<SiteHead, PrrRecord> | undefined, action: Action): Map<SiteHead, PrrRecord> {
