import { InputConfiguration, InputType } from 'shared';
...
import { TestOptionType, UserSettings } from './../../models/usersettings.model';
...
export class TestOptionComponent implements OnInit, OnDestroy {
  ...
  private updateTestOptions(userSettings: UserSettings): void {
    userSettings.testOptions.forEach(e => {
      const option = this.testOptions.find( o => o.type === e.type);
      if (option) {
  ...
  private updateTestOptionConfigs(): void {
  ...
  optionChanged(checkboxValue: boolean, testOption: TestOption): void {
