...
describe('TableComponent', () => {
  ...
  const tableRows = [[
    ...
    },
    {
      ...
      callBack: {
        editable: true,
        onUserInput: (_value: string): any => {return {};},
        valid: (_value: string): boolean => true
      }
    }
  ]];

  function arraysOfStringsAreEqual(first: Array<string>, second: Array<string>): boolean {
    const key1 = first?.reduce((a,c) => a + '_' + c, '');
    const key2 = second?.reduce((a,c) => a + '_' + c, '');
    return key1 === key2;
  }
  ...
  it('should support table header and rows', () => {
    ...
    const tableHeaderElement = debugElement.queryAll(By.css('.tableHeader li'));
    const tableRowElement = debugElement.queryAll(By.css('.tableRow li'));
  ...
  describe('Table Headers', () => {
    it('should support text', () => {
      ...
      const tableHeaderLabels = debugElement.queryAll(By.css('.tableHeader li')).map(e => e.nativeElement.innerText);
    ...
    it('should support backgroundColor', async () => {
      ...
      const tableheaderElement = debugElement.query(By.css('.tableHeader li')).nativeElement;
  ...
  describe('Table Rows', () => {
    it('should support texts', () => {
      ...
      const tableRowEntry = debugElement.queryAll(By.css('.tableRow li')).map(e => e.nativeElement.innerText);
    ...
    it('should support styles', () => {
      ...
      const tableRowEntryElement = debugElement.query(By.css('.tableRow li')).nativeElement;
  ...
  describe('initTable', () => {
    it('should support array of width', async () => {
      ...
      const firstTableEntries = debugElement.query(By.css('.tableRow')).queryAll(By.css('li'));
      const widthOfFirstTableEntries = firstTableEntries.map(e => e.nativeElement.style.width);
      await expectWaitUntil(
        () => fixture.detectChanges(),
